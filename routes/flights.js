...
router.get("/:flightNumber/:date", (req, res) => {
  ...
  fetch(
    `https://aerodatabox.p.rapidapi.com/flights/number/${req.params.flightNumber}/${req.params.date}`,
    {
      method: "GET",
      headers: {
    ...
    .then((data) => {
      if (data) {
        if (
          data[0].departure.revisedTime === undefined &&
          data[0].arrival.revisedTime === undefined
        ) {
          res.json({
            result: true,
            flightData: {
          ...
          data[0].departure.revisedTime &&
          data[0].arrival.revisedTime
        ) {
          res.json({
            result: true,
            flightData: {
